...
> [!NOTE]
> A lot of this code is applicable to analysing the TTree-based CAFs. What differs is how you open and grab the StandardRecords from the ROOT file. The biggest difference with RNTuple is the Python support, which no longer requires pySRProxy and is much faster.

In the Python approach, we use `uproot` together with `awkward` to analyse the CAFs. This approach is fast (comparable to C++) and can be very interactive, which is ideal for exploratory work. There is a [Jupyter notebook](python/awkward.ipynb) which takes you through some examples. 
